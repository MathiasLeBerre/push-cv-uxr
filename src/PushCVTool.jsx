...
import { Plus, Trash2, ToggleLeft, ToggleRight, Sparkles, Copy, Check, Mail, FileText, Building2, User, Briefcase, Globe, MessageSquare, AlertCircle, Loader2, X, Link2, Edit3, Save, Repeat2, ChevronDown, ChevronUp, Send } from 'lucide-react';
...
Génère le JSON de résultat pour ce contact.`;
  ...
  const handleReverse = async () => {
    ...
    const prompt = `Tu es l'assistant d'un Business Manager chez UX-Republic, agence de conseil UX/Product Design à Paris.\n\nUn consultant est disponible (${c.status === 'intercontrat' ? 'intercontrat — priorité business absolue' : 'vivier'}). Identifie 3 entreprises françaises (ou présentes en France) à prospecter en priorité pour ce profil.\n\n# PROFIL DU CONSULTANT\n- Nom : ${c.name}\n- Rôle : ${c.role}\n- Expérience : ${c.experience}\n- Secteurs forts : ${(c.sectorsStrong || []).join(', ')}\n- Expertises clés : ${(c.expertises || []).slice(0, 5).join(', ')}\n- Clients passés : ${(c.keyClients || []).slice(0, 5).join(', ')}\n- Points forts : ${(c.highlights || []).slice(0, 3).join(' | ')}\n\n# CONTRAINTE ABSOLUE\nCible UNIQUEMENT des entreprises utilisatrices finales (grands groupes, scale-ups, secteur public). JAMAIS d'ESN, agences digitales, cabinets conseil IT.\n\nPour chaque entreprise : interlocuteur ciblé, raison du match, mail direct (60-80 mots) commençant par "Bonjour [prénom]," et terminant EXACTEMENT par "Auriez-vous des disponibilités pour en discuter ?\\n\\nExcellente journée,\\nMathias"\n\n# FORMAT — JSON UNIQUEMENT, sans backticks\n{\n  "consultantSummary": "1-2 phrases",\n  "prospects": [\n    {\n      "company": "Nom",\n      "sector": "Secteur",\n      "targetContact": "Poste précis",\n      "matchReason": "1-2 phrases",\n      "email": { "subject": "Objet", "body": "Corps du mail" }\n    }\n  ]\n}`;
    try {
      const data = await callClaude({ model: 'claude-haiku-4-5', max_tokens: 2500, messages: [{ role: 'user', content: prompt }] }, apiKey);
      const text = data.content.filter(b => b.type === 'text').map(b => b.text).join('').replace(/```json|```/g, '').trim();
      const parsed = JSON.parse(text);
  ...
  const handleWish = async () => {
    ...
    const c = reverseConsultant;
    const prompt = `Tu es l'assistant d'un Business Manager chez UX-Republic, agence de conseil UX/Product Design à Paris.\n\nUn consultant exprime des souhaits. Identifie 3 entreprises à prospecter en croisant ses souhaits avec son profil réel.\n\n# PROFIL\n- Nom : ${c.name}\n- Rôle : ${c.role}\n- Expérience : ${c.experience}\n- Secteurs forts : ${(c.sectorsStrong || []).join(', ')}\n- Expertises clés : ${(c.expertises || []).slice(0, 5).join(', ')}\n- Clients passés : ${(c.keyClients || []).slice(0, 5).join(', ')}\n\n# SOUHAITS\n${wishText}\n\n# CONTRAINTE\nCible UNIQUEMENT des entreprises utilisatrices finales. JAMAIS d'ESN, agences digitales, cabinets conseil IT.\n\nPour chaque entreprise : poste à cibler, pourquoi ça matche, mail direct (60-80 mots) — "Bonjour [prénom]," — terminer EXACTEMENT par "Auriez-vous des disponibilités pour en discuter ?\\n\\nExcellente journée,\\nMathias"\n\n# FORMAT — JSON UNIQUEMENT, sans backticks\n{\n  "coherenceWarning": null,\n  "coherenceNote": null,\n  "prospects": [\n    {\n      "company": "Nom",\n      "sector": "Secteur",\n      "targetContact": "Poste précis",\n      "matchReason": "1-2 phrases",\n      "email": { "subject": "Objet", "body": "Corps du mail" }\n    }\n  ]\n}`;
    try {
      const data = await callClaude({ model: 'claude-haiku-4-5', max_tokens: 2500, messages: [{ role: 'user', content: prompt }] }, apiKey);
      const text = data.content.filter(b => b.type === 'text').map(b => b.text).join('').replace(/```json|```/g, '').trim();
      const parsed = JSON.parse(text);
...
Réponds UNIQUEMENT avec un JSON strict, sans markdown : { "subject": "...", "body": "..." }`;
    ...
    try {
      ...
      setRelanceSubject(parsed.subject || '');
      setRelanceEmail(parsed.body || '');
    } catch (e) {
      setRelanceEmail("Erreur lors de la génération. Réessaie.");
    }
    setRelanceLoading(false);
  };

  const copyRelance = () => {
    navigator.clipboard.writeText(relanceEmail);
                    ...
                    {isEditing ? (
                      ...
                      <div style={{ marginTop: 16 }}>
                        ...
                        <div style={{ display: 'flex', gap: 10 }}>
                          ...
                          <button onClick={(e) => { e.stopPropagation(); generateRelance(s); }} className="sans" style={{ flex: 2, padding: '10px', background: '#7A8C6F', color: '#FAF8F4', border: 'none', borderRadius: 999, cursor: 'pointer', fontSize: 13, fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><Send size={13} /> Générer relance</button>
                        ...
                        {/* Modale relance inline */}
                        {relanceId === s.id && (
                          <div style={{ marginTop: 16, padding: 16, background: '#F4F1E8', borderRadius: 16, border: '1px solid #E8E2D5' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                              <span className="sans" style={{ fontSize: 12, fontWeight: 600, color: '#1A1A1A', display: 'flex', alignItems: 'center', gap: 6 }}><Send size={12} /> Email de relance</span>
                              <button onClick={() => setRelanceId(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8B7E64' }}><X size={14} /></button>
                            </div>
                            {relanceLoading ? (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '20px 0', justifyContent: 'center' }}>
                                <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} color="#7A8C6F" />
                                <span className="sans" style={{ fontSize: 13, color: '#8B7E64' }}>Génération en cours…</span>
                              </div>
                            ) : relanceEmail ? (
                              <>
                                {relanceSubject && (
                                  <div style={{ marginBottom: 8 }}>
                                    <span className="sans" style={{ fontSize: 11, fontWeight: 600, color: '#8B7E64', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Objet</span>
                                    <div style={{ marginTop: 4, padding: '8px 12px', background: '#FFFFFF', borderRadius: 8, border: '1px solid #E8E2D5' }}>
                                      <span className="sans" style={{ fontSize: 13, color: '#1A1A1A' }}>{relanceSubject}</span>
                                    </div>
                                  </div>
                                )}
                                <span className="sans" style={{ fontSize: 11, fontWeight: 600, color: '#8B7E64', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Corps</span>
                                <textarea value={relanceEmail} onChange={e => setRelanceEmail(e.target.value)} className="sans" rows={7} style={{ width: '100%', marginTop: 4, padding: '10px 12px', border: '1px solid #E8E2D5', borderRadius: 10, fontSize: 13, resize: 'vertical', outline: 'none', fontFamily: "'Inter Tight', sans-serif", background: '#FFFFFF', boxSizing: 'border-box', lineHeight: 1.6 }} />
                                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                                  <button onClick={copyRelance} className="sans" style={{ flex: 1, padding: '9px', background: relanceCopied ? '#7A8C6F' : '#FFFFFF', color: relanceCopied ? '#FFFFFF' : '#1A1A1A', border: '1px solid #E8E2D5', borderRadius: 999, cursor: 'pointer', fontSize: 12, fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
